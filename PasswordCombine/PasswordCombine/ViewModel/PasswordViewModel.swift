...
class PasswordViewModel {
    ...
    func transform(input: Input) -> Output {
        ...
        let updatePublisher = input.newPasswordPublisher.flatMap { [unowned self] newPassword in
            ...
            return self.newPasswordValidation(text: newPassword)
            
        }.eraseToAnyPublisher()
        
        let resultPublisher = Publishers.CombineLatest(input.newPasswordPublisher, input.confirmPasswordPublisher).flatMap {  [unowned self] (newPassword, confirmPassword) in
            
            return self.confrimPasswordValidation(newPassword: newPassword, confirmPassword: confirmPassword)
    ...
    private func newPasswordValidation(text newPassword: String) -> AnyPublisher<Result, Never> {
    ...
    private func confrimPasswordValidation(newPassword: String, confirmPassword: String) -> AnyPublisher<Result, Never> {
